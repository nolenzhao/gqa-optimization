...
int main(int argc, char* argv[]) {
    ...
    if (std::stoi(argv[9]))
    {
        using namespace Mfma16x16PingPong;
        std::cout << "\n=== 16x16 LDS (with ping-ponging) ===\n";
        ...
        time_kernel("16x16-LDS pingpong", 
        [&](){
            gqa_packed<<<gridDim, blockDim>>>(
                d_queries, 
                d_keys, 
        ...
        // print_matrix(attention_output.data(), GROUP_SIZE, SEQ_LEN, PADDED_GROUP_SIZE, PADDED_SEQ_LEN, "16x16ldspingpong", true);
    ...
    if (std::stoi(argv[10]))
    {
        using namespace Mfma16x16HalfLDSPingPong;
        std::cout << "\n=== 16x16 LDS (with ping-ponging) ===\n";
        ...
        time_kernel("16x16Half-LDS pingpong", 
        [&](){
            gqa_packed<<<gridDim, blockDim>>>(
                d_queries, 
                d_keys, 
